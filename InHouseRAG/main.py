from DataHouse.Readers import GetDataFromNotionAPI
...
from Configs import KgRetrieverNotionDataLlamaIndex
...
class RAGBasedOnNotionData:
    ...
    def getData(self, data_args: dict = None) -> None:
        ...
        if 'notion_api_key' in os.environ.keys():
            gdn = GetDataFromNotionAPI(notion_api_key=os.environ['notion_api_key'], **data_args)
            gdn.get_data()
        else:
            raise ValueError(
...
def get_query_engine(arguments_config:dict=None):
    notion_model = RAGBasedOnNotionData()
...
if __name__ == '__main__':
    get_query_engine(arguments_config=KgRetrieverNotionDataLlamaIndex)
